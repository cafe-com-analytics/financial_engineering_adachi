...
small_symbols = {'AAL', 'AAPL', 'AAP', 'ABBV', 'ABC', 'ABT', 'ACN', 'ADBE', 'ADI',
       'ADM', 'ADP', 'ADSK', 'ADS', 'AEE', 'AEP', 'AES', 'AET', 'AFL',
       'AGN', 'AIG', 'AIV', 'AIZ', 'AJG', 'AKAM', 'ALB', 'ALGN', 'ALK'}
...
for symbol in small_symbols:
    f = f"./FinancialEng/data/interim/{symbol}.csv"
    if os.path.exists(f):
        df = pd.read_csv(f)
        df['Name'] = symbol
        if full_df is None:
...
full_df.to_csv('./FinancialEng/data/processed/sp500sub.csv', index=False)
